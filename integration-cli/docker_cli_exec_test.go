...
func TestExecWait(t *testing.T) {
	...
	cmd = exec.Command(dockerBinary, "wait", execID)
	out, _, err = runCommandWithOutput(cmd)

	ec := strings.TrimSpace(out)
	if ec != "29" {
		t.Fatal("Should have an ExitCode of 29, not: %v", ec)
	}
	...
	cmd = exec.Command(dockerBinary, "wait", "999000")
	out, _, _ = runCommandWithOutput(cmd)

	if !strings.Contains(out, "No such container or exec: 999000") {
	...
	cmd = exec.Command(dockerBinary, "wait", out1, out2, "999000")
	out, _, err = runCommandWithOutput(cmd)

	lines := strings.Split(strings.TrimSpace(out), "\n")

	if lines[0] != "11" || lines[1] != "12" || !strings.Contains(lines[2], "No such container or exec: 999000") {
...
func TestExecWaitKillContainer(t *testing.T) {
	...
	// wait gets back the correct response (-1)
	go func() {
		time.Sleep(2 * time.Second)
	...
	ec := strings.TrimSpace(out)
	if ec != "-1" {
		t.Fatal("Should have an ExitCode of -1, not: %v", ec)
	}
